...
class _BrowserSession:
    ...
    async def _get_page(self) -> Page:
        # Chromium contexts can disappear when the automation browser crashes,
        # is externally closed, or a persistent-profile launch is replaced.
        # A non-None context object is not proof that it is still usable.
        context_closed = False
        if self._context is not None:
            try:
                context_closed = bool(self._context.is_closed())
            except Exception:
                context_closed = True
        if context_closed:
            self._context = None
        ...
        if self._page is None or self._page.is_closed():
            try:
                ...
                # The context may have died between the check and new_page().
                # Rebuild the persistent context once before surfacing an error.
                self._context = None
                self._page = None
                await self._launch()
                self._page = await self._context.new_page()
    ...
    async def youtube_play(self, query: str = "") -> str:
        """Open the first normal YouTube result and explicitly start playback.

        This is used only for an explicit AXON play/listen command. It avoids
        brittle single-selector clicks and verifies that the HTML5 player is
        actually playing before reporting success. YouTube consent, ads,
        restrictions, and account policies remain outside AXON's control.
        """
        query = str(query or "music").strip() or "music"
        page = await self._get_page()
        ...
        try:
            ...
            # Dismiss only common non-destructive consent overlays if present.
            for selector in (
            ...
            # Prefer ordinary video renderers and explicitly exclude Shorts.
            candidates = page.locator('ytd-video-renderer a#thumbnail, ytd-video-renderer a[href*="/watch?v="]')
            ...
            if not target:
                # Fallback to any visible watch URL on the results page.
            ...
            if not target:
                return "No playable YouTube video result was found."
            ...
            # Attempt normal UI activation first.
            for selector in (
                ...
                try:
                    ...
                    if await loc.count():
                        ...
                        if selector != 'video' or await loc.is_visible():
            ...
            # Verify and, if necessary, request playback from the HTML5 element.
            ...
            if not playing:
                # A delayed player can become ready after the first check.
                await page.wait_for_timeout(1200)
                playing = await page.evaluate("""() => {
                    const v = document.querySelector('video');
            ...
            return "YouTube opened the selected video, but playback did not start; the browser or YouTube may require user activation."
        except Exception as exc:
            return f"YouTube playback error: {exc}"
...
def browser_control(
    ...
    try:
        ...
        if action == "click":
            ...
            result = sess.run(sess.youtube_play(params.get("query", "music")))
        else:
