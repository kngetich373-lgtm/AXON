...
def _screenshot(save_path: str | None = None) -> str:
    ...
    # On Wayland, pyautogui/pyscreeze may fail because scrot is X11-only.
    # Try native screenshot tools first.
    ...
    if is_wayland:
        for cmd in [
            ...
            if subprocess.run(["which", cmd[0]], capture_output=True).returncode == 0:
                ...
                return f"Screenshot command ran but file is empty: {path}"
        raise RuntimeError(
            "Wayland detected but no screenshot tool found. "
            "Install one: sudo apt install gnome-screenshot (GNOME) "
            "or grim (Sway/wlroots)."
        )
